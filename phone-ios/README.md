...
- A failed server join or reconnect does not stop local walking. Detected steps drive your pet and its gait immediately; multiplayer destinations are sent only while connected. Local walking can be tested while the server is unavailable or awaiting an update.

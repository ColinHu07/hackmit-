...
After installation, look for **Bondimals** with the green paw icon on your Home Screen. Allow location on first launch for automatic nearby discovery, or when starting walking, and camera/microphone when recording.
...
- Movement is scaled: 1 real meter equals 0.2 world units. The existing board spans -3 to +3 in X/Z. **Recenter** lets you continue at its edge. This is a north-aligned miniature world, not AR world anchoring or measured placement of other pets. Nearby visitors still use the explicitly illustrative arrangement.
...
Location sensors stop when all interested features stop, and both discovery and walking stop in the background. Discovery resumes on foreground return unless explicitly stopped; walking still requires an explicit restart. No location history is written to disk. Native privileges are exposed only to the app's bundled main frame; navigation to external pages is blocked.
...
Happiness is saved on this device, starts at 70%, and decreases 2 points per hour down to a gentle floor of 20%. Each completed quest step and mutually confirmed real-world hello adds 12 points (maximum 100%). Completion IDs are retained so reconnecting/reloading cannot reward the same quest again. These use the existing server quest events; camera clips are not AI-verified yet. Happiness is a local prototype statistic, not an account-synced or tamper-proof reward system.

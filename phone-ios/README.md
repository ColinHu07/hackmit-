...
- Movement is scaled: 1 estimated meter equals 0.2 world units before the step amplification above. Walking keeps continuous shared X/Z coordinates beyond the original meadow; it never rebases your pet to the center. Scenery repeats around the camera without moving players. The server advertises a ±10,000 safety boundary. Older servers still limit walking to ±3 and show an update notice. This is a miniature virtual world, not AR world anchoring or measured placement of other pets. Nearby visitors still use the explicitly illustrative arrangement.
...
- A failed server join or reconnect does not stop local walking. Offline steps drive the local preview immediately. While connected, both your pet and other pets render the same server positions with smoothing; compass updates change facing without replacing shared coordinates. Steps taken during a temporary disconnect are submitted on reconnection. Local walking can be tested while the server is unavailable or awaiting an update.
- Rendered movement carries velocity between detected footfalls, then settles at the last detected position when you stop. The alternating foot animation follows that smooth ground travel. Seeded flower patches, grass, mushrooms, low shrubs, and scattered rocks replace the square border; these landmarks stay anchored as the camera follows you. Plants use instancing to keep phone rendering lightweight.

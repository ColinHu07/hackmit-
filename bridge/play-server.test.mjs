...
test('two authenticated phone players persist shared play rewards and pet state', async t => {
  const { origin, connect } = await setup(t);
  const register = async name => {
    const response = await fetch(origin + '/api/v1/players', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name }) });
    assert.equal(response.status, 201);
    return response.json();
  };
  const profile = async token => {
    const response = await fetch(origin + '/api/v1/me', { headers: { authorization: `Bearer ${token}` } });
  ...
  duplicate.send({ type: 'join', roomCode: first.roomCode, name: 'Alice again', accountToken: alice.token });
  await error(duplicate, 'room_full');
  a.send({ type: 'move', x: 0, z: 0 });
  b.send({ type: 'move', x: 0, z: 0 });
  await state(a, snapshot => snapshot.quest.met);
  a.send({ type: 'action', action: 'play' });
  await state(b, snapshot => snapshot.bond === 1);
  const aProfile = await profile(alice.token);
  const bProfile = await profile(bob.token);
  assert.equal(aProfile.rewards.interactions, 1);
  assert.equal(bProfile.rewards.interactions, 1);
  assert.equal(aProfile.pet.happiness > 70, true);
  assert.equal(bProfile.pet.happiness > 70, true);

...
export function cameraAvailability(state: CaptureStatus): { ready: boolean; title: string; message: string } {
  ...
  return { ready: false, title: titles[state.cameraState ?? 'idle'], message: state.cameraMessage || 'Open Kith Camera on your iPhone, start the glasses camera, and keep the app open. Update the phone app if it still shows Camera 4 or earlier.' };
}

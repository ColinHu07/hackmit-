...
Camera 5 shows its build number and actual camera readiness at the top of the phone screen. Manual pairing starts the camera through the same Meta registration/permission flow as a setup link. The camera bearer and matching server origin are saved in this phone's non-synchronizing, unlocked-only Keychain; opening the app checks that saved authorization with the server before automatically starting the camera. Unpairing, changing servers, or a server 401 clears it. Pairing codes, game-player tokens, and media are not persisted. A server restart or expired game membership still requires a new code.

The bridge sends `/glasses/heartbeat` about once per second. Ready requires an active phone app, a streaming DAT camera and an image younger than two seconds. During an explicitly requested clip only, `/glasses/progress` sends at most two 320px previews per second, each below 100 KiB; at most one upload is in flight and slow networks drop previews without blocking evidence collection. Canceling, backgrounding, stopping the camera, or finishing the clip stops preview uploads. The final photo or twelve-frame clip continues through the existing result and grading flow. These previews are separate from optional desktop hand-tracking preview.

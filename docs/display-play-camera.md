...
Swipe/arrow to focus; pinch/Enter activates once. Walking starts automatically when the player joins. APIs without an explicit permission request attach immediately. If a browser requires a user gesture for sensor permission, the first normal interaction requests it; Allow motion remains available as a fallback. Explicit Pause persists across reloads, and Resume restores motion. Wave, Berry and Quests stay in the main rail. Photo and Record clip open a dedicated recorder. Pairing leaves the camera idle; an explicit capture starts it, records, then stops the camera and parent DAT session. The glasses may temporarily hide the Web App while DAT owns the session. Reopen Kith from the glasses app grid (or Resume in its menu) to return to the selected quest with the saved preview, **Submit to Muse**, and **Retake clip/photo**. Opening the same quest restores its saved capture; a separate review screen is no longer required. Capture never grades automatically. Up to two live preview frames per second appear only if the firmware keeps the web display active. Previously enabled movement resumes on foreground return with fresh samples; explicit Pause keeps it off.
...
The simulator is `?simulator`: motion starts automatically, W advances a synthetic step, A/D turn; it never labels these as physical glasses measurements. Relative heading initially aligns to the server character. Physical testing on the user's Meta glasses confirmed the `+1` heading sign: a right turn steers the beaver right without an inversion setting. Recenter keeps position. Head-facing is explicitly locked on the server during walking so other clients see it too.
...
| `POST /glasses/heartbeat` | Camera bearer plus `{cameraReady, cameraState, onDemandCapture?, message?}` | Reports actual camera readiness: `ready`, `starting`, `permission`, `paused`, `error` or `idle`. Readiness expires after five seconds. `onDemandCapture: true` advertises native Camera 8 ability to start a camera for a command without keeping DAT active. |
...
The bridge **never** calls the quest verifier. Capturing delivers private evidence to the authenticated game client; only the explicit **Submit to Muse** button starts its grading through `/verify`. Status returns the current request's quest and kind so a recreated display page can restore that quest. Stale responses cannot restore a discarded or replaced request. Generic transport failures preserve evidence for recovery; explicit Cancel/Discard erase it. Preview images are not written to SQLite or disk. The chosen HTTPS host handles transient camera traffic.

## Device validation still required

Physical testing on this device showed that recording can blank the standalone Web App while the camera remains active. Camera 8 releases both camera and DAT session after every bounded capture, and Meadow 8 restores the result after reconnect/reload. Automatic display return is not promised; use the Web App menu's Resume action or select Kith again from the glasses app grid. [Official setup](https://wearables.developer.meta.com/docs/develop/webapps/setup/) and [session lifecycle](https://wearables.developer.meta.com/docs/develop/dat/lifecycle-events/).

1. Pair Camera 8 and confirm it reports ready for quests with the stream closed.
2. Select Record clip. If Kith disappears, let recording finish, then reopen Kith promptly.
3. Confirm the camera/session stops, the selected quest appears with the saved clip, and no grade occurs before Submit to Muse.
4. Submit once; confirm the quest result. Repeat using Retake and verify the replacement stays in the same quest and no grade occurs before Submit.
...
Meadow 8 retries authenticated status/evidence reads for up to 90 seconds after returning to the display, with up to 45 seconds per download bounded by the remaining deadline. Network interruption, timeout, interrupted response bodies, and temporary gateway failures are recoverable; authorization and invalid-data errors remain actionable. Capture and discard are never automatically repeated. Opening a quest restores its saved evidence and shows connection feedback in that quest. Leaving that view, changing player/session, or starting Submit/Discard prevents an older status response from repainting the interface.

Camera 8 retains the same encoded capture for up to 20 seconds while retrying transient result-upload failures, with at most six seconds per attempt. The server acknowledges an identical upload for the same current request without replacing evidence, extending retention, or grading again. Changed, discarded, expired, replaced or unauthorized results remain rejected. Separate sanitized last-capture diagnostics record recording, upload, acknowledgment and failure without including media or credentials.

## Saved capture submission

Meadow 8 submits a capture request ID instead of uploading its downloaded frames again. The server resolves the authenticated owner’s retained capture and acknowledges an asynchronous Muse job immediately. A stable submission ID makes a lost acknowledgment safe to retry; status requests retrieve the existing result without grading twice. Camera status includes only grading that the user explicitly started, so reloads can resume it without starting new verification. Provider failures remain visible instead of becoming generic tunnel errors; another explicit Submit can retry a failed grade. Retake starts a new capture of the same kind in the selected quest.

## Startup and clip duration

The display distinguishes camera startup from recording and saving. Only actual capture progress produces a six-second recording countdown; a live camera heartbeat alone does not mean recording has begun. The native recording window is six monotonic seconds from fresh camera readiness, with at most twelve samples collected inside that window. Camera connection/permission, session release, and upload add time outside that recording interval. Previously, waiting for twelve frames could extend collection toward nine seconds. Startup failures preserve the actual phase rather than blaming camera permission for every timeout.

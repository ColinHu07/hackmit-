...
- Runtime SHA-256: `3ec7922d88f741eb17b3b8f2fd98f73a5915820f55c8d5375ff60ccd8a133eff`.
...
| --- | --- | --- |
| Distinct RGBA values | 710,298 | 34,004 |
...
Simplification remains lossy: retained vertices keep their exact source positions; colors remain unchanged outside the cheek repair, with interpolated colors across larger triangles. The optimized GLB is approximately 97.3% smaller than the source, with bounding-box extrema differing by less than 0.0001 source units. It retains the source's two connected components; the result has no boundary edges, nonmanifold edges, or degenerate triangles. The source itself contained 30 nonmanifold edges. No new facial parts or surface smoothing were added. A targeted color repair removes the source model’s stray white/black eye projection on the right cheek.
...
## Right-eye paint repair

The source GLB has an extra white patch with a dark outline below the right eyeball. It is mostly hidden head-on but looks like a stretched or duplicated eye from the overhead game camera. After simplification, `scripts/repair-character-eye.mjs` blends 234 cheek vertices back into the nearby orange fur palette. Its spatial mask follows the crease beneath the eye and feathers into the muzzle. The pupils, catchlights, mesh topology, material, alpha, bounds, and file size are unchanged. This correction is included in the normal rebuild command and does not modify the supplied source file.

Verified the exported asset side by side from front, overhead, and three-quarter views. Appearance regression checks cover the corrected cheek color as well as both pupils, catchlights, opaque closed topology, and the existing animation rig checks.

...
Native phones and tablets use the original step-based WalkingTracker movement again. Each detected step moves 0.35 world units (the existing 2.5× visual stride gain), with GPS used only as a fallback when step tracking is unavailable. Heading and tilt still control the view. Mobile scene taps cannot create destinations; desktop pointer/keyboard movement remains available. Touch browsers can explicitly enable walking.

The server retains support for `location` and `steps` messages from previously installed clients, but the restored client sends WalkingTracker destinations through `move`. Play together still uses the same 3-world-unit proximity radius in the client and server; one press starts the dance for nearby pets. With step-based movement, this radius measures game positions rather than a reliable physical distance.

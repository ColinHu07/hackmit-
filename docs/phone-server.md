...
- For a high five, send `interaction_invite` with a unique `requestId`, `kind: "high_five"`, and the other player's `targetPlayerId`. The other player replies with `interaction_respond`, the interaction ID, and `accept: true|false`. Both pets must be within 1.5 virtual units; invitations expire after 15 seconds. The snapshot reports the outcome and whether linked accounts received rewards.
- To view the same virtual room on glasses, select **Pair glasses display** on the phone. Open `/shared.html` in the glasses app and enter the `/play` server address and the one-use code shown on the phone. The code lasts 60 seconds. The display can observe both pets but cannot move or act for the phone player. This is a virtual meadow view, not physical AR alignment.
